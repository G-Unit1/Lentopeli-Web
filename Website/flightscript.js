...
async function fetch_login(username, password) {
    try {
        return fetch(
            `https://make-s.duckdns.org:15486/continue_game/${username},${password}`).then(response => response.json());
    } catch (error) {
...
async function fetch_player(player_name) {
    try {
        return fetch(`https://make-s.duckdns.org:15486/get_player/${player_name}`).then(response => response.json());
...
async function fly_to(username, airport) {
    try {
        await fetch(
            `https://make-s.duckdns.org:15486/fly_to/${username},${airport}`).then(response => response.json());
    } catch (error) {
        console.log(error.message);
    }
}
